...
也可以访问我们的关于页面：https://ckckh2023.github.io/BTIR-BrainTumor-ImageRecognition/
...
| 结果展示 | 首屏展示双模型综合结论、随访对比和关键影像特征，下方提供切片概率曲线、分割叠加和按需展开的区域明细 |
| 3D 查看 | 切换四模态、三视图与体渲染，叠加预测掩码并调整透明度 |
| 随访与任务 | 按病例分组历史检查并选择两次结果对比，支持异步提交、进度轮询、取消、失败重试、运行历史、归档与恢复 |
...
6. 按“综合结果、随访对比、影像学关键特征、模型依据”的顺序查看分析结果
7. 展开分割区域明细或“查看详细数据”获取定量字段和文件信息
8. 在“3D查看”中切换模态、查看分割掩码或体渲染
9. 对同一病例提交复查影像后，在分析页选择历史检查进行对比
...
- AI 辅助分析只解读上述结构化信息，成功时展示结论、建议和观察项，并在前端标出实际提供方
- AI 服务不可用不会影响本地分类、分割和综合结论
...
浏览器端到端测试使用本地模拟 API，覆盖登录、ZIP 上传、异步任务提交、重试、取消与 3D 查看入口，不依赖模型权重、Redis 或 AI 接口：
