...
结论。任务流程会临时复用现有 2D 分类器做 FLAIR 轴向切片集成，但该输出带有
`experimental=true`，不能用于跳过分割，也不等同于经过患者级验证的原生 3D
分类模型。后续原生 3D 分类器仍应作为独立适配器接入。
...
`WT={1,2,4}`、`TC={1,4}`、`ET={4}` 的 Dice，并记录耗时和 CUDA 峰值显存。
双空区域不计入该区域数据集均值，避免空标签人为抬高成绩。默认只保留 JSON
报告；通过 `--predictions-dir <目录>` 可以同时保留预测掩码。
