...
class THPAEncFR3(nn.Module):
    def __init__(self, in_channels, expr):
        super().__init__()
        # Training keeps the paper implementation's random directional branch.
        # In evaluation mode the inference loader assigns a stable direction to
        # every block so identical inputs cannot change with the global RNG.
        self.inference_direction = 0
    ...
    def forward(self, input_x: Tensor, dummy_tensor=None):
        ...
        if self.training:
            ...
            random_direction = int(self.inference_direction)
            if random_direction not in (0, 1, 2):
                raise ValueError(
                    f"inference_direction must be 0, 1, or 2; got {random_direction}"
                )
