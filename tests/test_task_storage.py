...
class TaskStorageTests(unittest.TestCase):
    ...
    @staticmethod
    def _record(task_id: str, status: str = "created") -> dict[str, object]:
        return {
            "task_id": task_id,
            "name": f"测试任务 {task_id}",
            "status": status,
    ...
    def test_save_load_and_update_task(self) -> None:
        ...
        self.assertEqual(self.repository.load(task_dir)["status"], "created")

        record = self.repository.load(task_dir)
        record["status"] = "queued"
        self.repository.save(task_dir, record)

        self.assertEqual(self.repository.load(task_dir)["status"], "queued")
